...
int main(int argc, const char* argv[])
{
    ...
    struct TinyCSocketAddressInfo* remote_info = NULL;

    struct TinyCSocketAddressInfo hints = { 0 };
    hints.ai_family = TINYCSOCKET_AF_INET;
    hints.ai_socktype = TINYCSOCKET_SOCK_DGRAM;

    tinycsocket_getaddrinfo("localhost", "1212", &hints, &remote_info);

    bool didConnect = false;
    for (struct TinyCSocketAddressInfo* address_iterator = remote_info; address_iterator != NULL; address_iterator = address_iterator->ai_next)
    {
        ...
        if (tinycsocket_connect(socket, address_iterator->ai_addr, address_iterator->ai_addrlen) != TINYCSOCKET_SUCCESS)
            continue;
    ...
    if (tinycsocket_send(socket, msg, sizeof(msg), 0, NULL) != TINYCSOCKET_SUCCESS)
        return show_error("Could not send message");

    uint8_t recv_buffer[1024];
    size_t bytes_recieved = 0;
    if (tinycsocket_recv(socket, recv_buffer, sizeof(recv_buffer) - sizeof('\0'), 0, &bytes_recieved) != TINYCSOCKET_SUCCESS)
