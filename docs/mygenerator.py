...
FunctionData = collections.namedtuple(
    'FunctionData', ['name', 'refid', 'brief', 'argsstring', 'definition', 'params', 'returns', 'seealso'])
...
def get_function_descriptons(doxygen_folder: Path):
    ...
    for file in all_files:
        ...
        for f in fn:
            ...
            param_description_nodes = f.findall('./detaileddescription//parameteritem/parameterdescription/para')
            param_descriptions = [xml_to_rst(x) for x in param_description_nodes]
            param_description_name_nodes = f.findall('./detaileddescription//parameteritem/parameternamelist/parametername')
            ...
            for p in params_symbols:
                if p[1] in param_description_names:
                    params.append((p[0], p[1], param_descriptions[param_description_names.index(p[1])]))
                else:
            ...
            m.append(FunctionData(name, refid, brief, argsstring, definition, params, returns, seealso))
    return m
...
def run():
    ...
    m = get_function_descriptons(Path(dir, './_doxyxml/'))
